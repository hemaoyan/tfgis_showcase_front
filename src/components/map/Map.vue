<template>
  <div :id="`tfgis-container${mapKey}`"
    :class="['tfgis-container', customClass, { 'tfgis-container-compare-rh' : compare }]"></div>
</template>
...
export default {
  ...
  mounted() {
    ...
    if (this.mapKey) {
      this.inittfgis(this.options)
    } else {
      mars3d.Resource.fetchJson({ url: this.url }).then((data) => {
        this.inittfgis(data.map3d)// 构建地图
      })
    }
  },
  ...
  methods: {
    inittfgis(options) {
      ...
      // 创建三维地球场景
      var map = initMap(`tfgis-container${this.mapKey}`, mapOptions)
